...
import { registerAdmin, getAllUsers, getUserById, updateUser, deleteUser } from '../controllers/user.controller.js';
import { authMiddleware, roleMiddleware } from "../middlewares/authMiddleware.js";

const router: Router = Router();
router.get('/', getAllUsers);
router.get('/:id', authMiddleware, roleMiddleware('manager'), getUserById);
router.post('/', authMiddleware, roleMiddleware('manager'), registerAdmin);
